...
#define NIBBLE_TO_CHR(x) Nibble2ChrTable[x]

void str_hex_b(_WString* s, unsigned int x)
{
	/*
	 * def prebuilt():
	 * 	s = ""
...
void str_code_hb(_WString* s, unsigned int x)
{
...
void str_code_hdw(_WString* s, uint32_t x)
{
	int8_t* buf;
	int i = 0, shift = 0;
	unsigned int t = 0;
	...
	for (shift = 28; shift != 0; shift -= 4) {
		t = (x >> shift) & 0xf;
		if (i | t) buf[i++] = NIBBLE_TO_CHR(t);
	}
	t = x & 0xf;
	buf[i++] = NIBBLE_TO_CHR(t);
	
	s->length += i + 2;
	buf[i] = '\0';
}

void str_code_hqw(_WString* s, uint8_t src[8])
{
	int8_t* buf;
	int i = 0, shift = 0;
...
void str_off64(_WString* s, OFFSET_INTEGER x)
{
	int8_t* buf;
	int i = 0, shift = 0;
	OFFSET_INTEGER t = 0;
...
#endif /* DISTORM_SUPPORT_64BIT_OFFSET */
