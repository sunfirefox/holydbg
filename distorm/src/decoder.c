...
static _DecodeResult decode_inst(_CodeInfo* ci, _PrefixState* ps, _DInst* di)
{
	...
	if ((lockable == DISTORM_TRUE) && (instFlags & INST_PRE_LOCK)) {
...
#ifdef DISTORM_DISABLE_BACKWARD_COMPATIBILITY
			...
			if ((codeLen == 0) || (prefixSize == INST_MAXIMUM_SIZE)) {
				if (~_ci->features & DF_RETURN_FC_ONLY) {
					...
					for (p = code; p < ps.last; p++, startInstOffset++) {
						/* Use next entry. */
#ifndef DISTORM_DISABLE_BACKWARD_COMPATIBILITY
						if (supportOldIntr) {
							pdi = (_DInst*)((char*)result + nextPos * sizeof(_DecodedInst));
						}
						else
#endif /* DISTORM_DISABLE_BACKWARD_COMPATIBILITY */
						{
							pdi = &result[nextPos];
						}
						nextPos++;
