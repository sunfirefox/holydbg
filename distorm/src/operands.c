...
static DISTORM_FASTCALL unsigned int operands_fix_8bit_rex_base(unsigned int reg)
{
...
static DISTORM_FASTCALL void operands_set_ts(_Operand* op, _OperandType type, uint16_t size)
{
	op->type = type;
	op->size = size;
}
...
static DISTORM_FASTCALL void operands_set_tsi(_Operand* op, _OperandType type, uint16_t size, unsigned int index)
{
	op->type = type;
...
static DISTORM_FASTCALL int read_stream_safe_uint(_CodeInfo* ci, void* result, unsigned int size)
{
	ci->codeLen -= size;
	if (ci->codeLen < 0) return DISTORM_FALSE;
	switch (size)
	{
...
static DISTORM_FASTCALL int read_stream_safe_sint(_CodeInfo* ci, int64_t* result, unsigned int size)
{
	ci->codeLen -= size;
	if (ci->codeLen < 0) return DISTORM_FALSE;
	switch (size)
                                  ...
                                  _IFlags instFlags)
{
	...
	if (lockableInstruction && (ps->decodedPrefixes & INST_PRE_LOCK)) *lockableInstruction = DISTORM_TRUE;
	...
	if (effAdrSz == Decode16Bits) {
		...
		if ((mod == 0) && (rm == 6)) {
			...
			op->type = O_DISP;
			di->dispSize = 16;
			if (!read_stream_safe_sint(ci, (int64_t*)&di->disp, sizeof(int16_t))) return DISTORM_FALSE;
		} else {
			/*
			...
			if (mod == 1) { /* 8 bits displacement + indirection */
				di->dispSize = 8;
				if (!read_stream_safe_sint(ci, (int64_t*)&di->disp, sizeof(int8_t))) return DISTORM_FALSE;
		...
		if ((mod == 0) && (rm == 5)) {
			...
			di->dispSize = 32;
			if (!read_stream_safe_sint(ci, (int64_t*)&di->disp, sizeof(int32_t))) return DISTORM_FALSE;
			...
			if (rm == 4) {
				...
				/* Read SIB byte. */
				if (!read_stream_safe_uint(ci, &sib, sizeof(int8_t))) return DISTORM_FALSE;
 ...
 * Return code:
 * DISTORM_TRUE - continue parsing the instruction and its operands, everything went right 'till now.
 * DISTORM_FALSE - not enough bytes, or invalid operands.
 */
                     ...
                     _DecodeType effAdrSz, int* lockableInstruction)
{
	...
	unsigned int vrex = ps->vrex, typeHandled = DISTORM_TRUE;
	_Operand* op = &di->ops[opNum];
	...
	switch (type)
	{
		...
		case OT_MEM_OPT:
			...
			if (mod == 0x3) return DISTORM_TRUE;
		break;
		...
		default: typeHandled = DISTORM_FALSE; break;
	}
	if (typeHandled) {
		/* All of the above types can't use a general-purpose register (a MOD of 3)!. */
		if (mod == 0x3) {
			if (lockableInstruction) *lockableInstruction = DISTORM_FALSE;
			return DISTORM_FALSE;
		}
	...
	switch (type)
	{
		case OT_IMM8:
			...
			if (!read_stream_safe_uint(ci, &di->imm.byte, sizeof(int8_t))) return DISTORM_FALSE;
		break;
		...
		case OT_IMM16: /* Force 16 bits imm. */
			...
			if (!read_stream_safe_uint(ci, &di->imm.word, sizeof(int16_t))) return DISTORM_FALSE;
		break;
			/*
			 ...
			 */
				...
				if (!read_stream_safe_uint(ci, &di->imm.qword, sizeof(int64_t))) return DISTORM_FALSE;
				break;
		...
		case OT_IMM32:
			...
			if (ci->dt == Decode64Bits) {
				...
				if (!read_stream_safe_sint(ci, &di->imm.sqword, sizeof(int32_t))) return DISTORM_FALSE;
			} else {
				op->size = 32;
				if (!read_stream_safe_uint(ci, &di->imm.dword, sizeof(int32_t))) return DISTORM_FALSE;
			}
		break;
		case OT_SEIMM8: /* Sign extended immediate. */
			...
			di->flags |= FLAG_IMM_SIGNED;
			if (!read_stream_safe_sint(ci, &di->imm.sqword, sizeof(int8_t))) return DISTORM_FALSE;
		break;
		case OT_IMM16_1:
			operands_set_ts(op, O_IMM1, 16);
			if (!read_stream_safe_uint(ci, &di->imm.ex.i1, sizeof(int16_t))) return DISTORM_FALSE;
		break;
		case OT_IMM8_1:
			operands_set_ts(op, O_IMM1, 8);
			if (!read_stream_safe_uint(ci, &di->imm.ex.i1, sizeof(int8_t))) return DISTORM_FALSE;
		break;
		case OT_IMM8_2:
			operands_set_ts(op, O_IMM2, 8);
			if (!read_stream_safe_uint(ci, &di->imm.ex.i2, sizeof(int8_t))) return DISTORM_FALSE;
		break;
		case OT_REG8:
		...
		case OT_CREG:
			...
			if ((reg >= CREGS_MAX) || (reg == 1) || ((reg >= 5) && (reg <= 7))) return DISTORM_FALSE;
		...
		case OT_DREG:
			 ...
			 */
			if ((reg == 4) || (reg == 5) || (vrex & PREFIX_EX_R)) return DISTORM_FALSE;
		...
		case OT_SREG: /* Works with REG16 only! */
			...
			if (lockableInstruction && (reg == 1)) return DISTORM_FALSE; /* Can't MOV CS, <REG>. */
			/*Don't parse if the reg exceeds the bounds of the array. */
			if (reg <= SEG_REGS_MAX - 1) operands_set_tsi(op, O_REG, 16, SREGS_BASE + reg);
			else return DISTORM_FALSE;
		break;
		case OT_SEG:
			op->type = O_REG;
		...
		case OT_RELC_FULL:
			...
			if (type == OT_RELCB) {
				...
				if (effOpSz == Decode16Bits) {
					...
					if (!read_stream_safe_sint(ci, &di->imm.sqword, sizeof(int16_t))) return DISTORM_FALSE;
		...
		case OT_MOFFS_FULL:
			...
			if (effAdrSz == Decode16Bits) {
				...
				di->dispSize = 16;
				if (!read_stream_safe_uint(ci, &di->disp, sizeof(int16_t))) return DISTORM_FALSE;
			} else if (effAdrSz == Decode32Bits) {
				ps->usedPrefixes |= INST_PRE_ADDR_SIZE;

				di->dispSize = 32;
				if (!read_stream_safe_uint(ci, &di->disp, sizeof(int32_t))) return DISTORM_FALSE;
			} else { /* Decode64Bits */
				di->dispSize = 64;
				if (!read_stream_safe_uint(ci, &di->disp, sizeof(int64_t))) return DISTORM_FALSE;
			}
		break;
		case OT_CONST1:
			operands_set_ts(op, O_IMM, 8);
