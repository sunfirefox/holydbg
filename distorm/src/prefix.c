...
int prefixes_is_valid(unsigned int ch, _DecodeType dt)
{
	switch (ch) {
		...
		case PREFIX_LOCK: return DISTORM_TRUE;
		case PREFIX_REPNZ: return DISTORM_TRUE;
		case PREFIX_REP: return DISTORM_TRUE;
		case PREFIX_CS: return DISTORM_TRUE;
		case PREFIX_SS: return DISTORM_TRUE;
		case PREFIX_DS: return DISTORM_TRUE;
		case PREFIX_ES: return DISTORM_TRUE;
		case PREFIX_FS: return DISTORM_TRUE;
		case PREFIX_GS: return DISTORM_TRUE;
		case PREFIX_OP_SIZE: return DISTORM_TRUE;
		case PREFIX_ADDR_SIZE: return DISTORM_TRUE;
		...
		case PREFIX_VEX3b: return DISTORM_TRUE;
	}
...
void prefixes_decode(const uint8_t* code, int codeLen, _PrefixState* ps, _DecodeType dt)
{
	 ...
	 */
	for (index = 0, done = DISTORM_FALSE;
		...
		*/
		...
		switch (*code)
		{
			...
			case 0x4f:
				...
				} else done = DISTORM_TRUE; /* If we are not in 64 bits mode, it's an instruction, then halt. */
			break;
			...
			default: done = DISTORM_TRUE; break;
		}
		if (done) break;
	}
